...
# Minimum bullish-vs-bearish separation.
MIN_DIRECTIONAL_SEPARATION = 5.0

# Direction must also beat neutral by this amount.
...
def analyze_context(candles):
    ...
    if total_score <= 0:
        ...
        direct_distribution = {
            ...
            "bullish":
                bullish_score / total_score * 100.0,

            "bearish":
                bearish_score / total_score * 100.0,

            "neutral":
                neutral_score / total_score * 100.0,
        }
...
def predict_from_context(context):
    ...
    # IMPORTANT FIX:
    #
    # A directional prediction is allowed only when:
    #
    # 1. Bullish and bearish are sufficiently separated.
    # 2. The strongest direction also beats neutral.
    #
    # Previously the engine only checked bullish vs bearish.
    # --------------------------------------------------------
    ...
    if directional_separation < MIN_DIRECTIONAL_SEPARATION:

        return "neutral"

    if neutral_gap < MIN_DIRECTIONAL_OVER_NEUTRAL:
...
def build_historical_records(candles):
    ...
    for decision_index in range(
        ...
        for horizon in HORIZONS:
            ...
            actual_direction = classify_outcome(
                movement
            )
...
# ============================================================
# DIRECTIONAL-ONLY STATISTICS
# ============================================================

def calculate_directional_statistics(records):
...
def context_match_score(
    ...
    # Exact match now includes direction.
    if (
        structure_match
        and momentum_match
    ...
    # Similar match requires the major structural
    # characteristics plus direction.
    if (
        structure_match
        and momentum_match
        and volatility_match
...
def calculate_failure_learning(
    ...
    # Only directional predictions participate in
    # directional failure learning.
    directional_records = [
        record
        for record in similar_records
        if record["prediction"] in (
...
def build_failure_patterns(
    ...
    for key, failures in pattern_failures.items():

        resolved = pattern_resolved[key]
...
def normalize_distribution(
    ...
    bullish = max(
        0.0,
        safe_float(bullish, 0.0)
    )

    bearish = max(
        0.0,
        safe_float(bearish, 0.0)
    )

    neutral = max(
        0.0,
        safe_float(neutral, 0.0)
    )
...
def determine_final_direction(
    ...
    # CRITICAL PROTECTION
    #
    # If neutral is stronger than the best directional
    # evidence, the engine MUST NOT call bullish/bearish.
    # --------------------------------------------------------

    if neutral >= strongest_direction:

        return (
            "neutral",
            "neutral_evidence_dominant",
    ...
    if neutral_gap < (
        ...
        return (
            "neutral",
            "direction_does_not_clear_neutral",
            directional_separation,
            neutral_gap,
        )
...
try:
    ...
    historical_records = (
        build_historical_records(candles)
    )
    ...
    historical_stats = calculate_statistics(
        historical_records
    )

    historical_correct = historical_stats["correct"]
    historical_incorrect = historical_stats["incorrect"]
    historical_accuracy = historical_stats["accuracy"]

    directional_historical_stats = (
        calculate_directional_statistics(
            historical_records
        )
    )

    directional_historical_accuracy = (
        directional_historical_stats["accuracy"]
    )
    ...
    if directional_historical_accuracy < (
    ...
    bullish_direct = direct["bullish"]
    bearish_direct = direct["bearish"]
    neutral_direct = direct["neutral"]
    ...
    weighted_historical_records = (
        historical_evidence["weighted_records"]
    )
    ...
    bullish_final = (
        bullish_direct * current_weight
        + historical_bullish * history_weight
    )

    bearish_final = (
        bearish_direct * current_weight
        + historical_bearish * history_weight
    )

    neutral_final = (
        neutral_direct * current_weight
        + historical_neutral * history_weight
    )
    ...
    if matched_resolved_count >= MIN_FAILURE_SAMPLE:

        sample_factor = min(
            matched_resolved_count / 200.0,
            1.0
        )
    ...
    #
    # 1. Bullish vs bearish separation.
    # 2. Direction vs neutral separation.
    #
    directional_confidence = (
        directional_separation
    )

    neutral_confidence = max(
        0.0,
        neutral_gap
    )
    ...
    if historical_influence <= 0:

        evidence_confidence *= 0.75

    if directional_historical_accuracy < 50.0:
    ...
    if integrated_direction == "neutral":
        ...
        quality_message = (
            "WARNING: Directional evidence does not "
            "clearly dominate neutral evidence."
        )

    elif directional_separation < STRONG_DIRECTION_SEPARATION:
    ...
    learning_memory = {
        ...
        "methodology": {
            ...
            "directional_prediction_requires_neutral_clearance":
                True,
    ...
    print(
        ...
        f"{directional_historical_accuracy:.1f}%"
    )
    ...
    if directional_historical_accuracy < 50.0:
        ...
        print(
            "WARNING: Directional historical "
        ...
        print(
            "PASS: Directional historical accuracy "
            "is at or above 50%."
        )
    ...
    if integrated_direction == "neutral":
        ...
        print(
            "WARNING: Directional evidence does "
            "not dominate neutral evidence."
        )

        print(
            "PASS: Neutral protection prevented "
            "an unsupported bullish/bearish decision."
        )
    ...
    print(
        "A directional decision must beat "
        "neutral evidence."
    )
...
- Direction vs neutral: {neutral_gap:.1f} percentage points
...
- Overall accuracy: {historical_accuracy:.1f}%
- Directional accuracy: {directional_historical_accuracy:.1f}%
...
- Matched directional records: {matched_resolved_count}
...
- Aggregate statistics are not treated as individual historical records.
...
- Direction must beat neutral evidence.
