...
class MarketStateBuilder:
    ...
    def _build_sequence(self, candles: List[CanonicalCandle], index: int) -> SequenceState:
       ...
       def _build_structure(
        ...
        for i in range(2, len(window) - 2):
            ...
            is_swing_high = (
                current.high > window[i - 1].high
                and current.high > window[i - 2].high
                and current.high > window[i + 1].high
                and current.high > window[i + 2].high
            )

            is_swing_low = (
                current.low < window[i - 1].low
                and current.low < window[i - 2].low
                and current.low < window[i + 1].low
                and current.low < window[i + 2].low
            )

            if is_swing_high:
                swings_high.append(
                    (current.timestamp, current.high)
                )

            if is_swing_low:
                swings_low.append(
                    (current.timestamp, current.low)
                )

        hh = (
            len(swings_high) >= 2
            and swings_high[-1][1] > swings_high[-2][1]
        )

        hl = (
            len(swings_low) >= 2
            and swings_low[-1][1] > swings_low[-2][1]
        )
        ...
        return MarketStructure(
            swing_highs=swings_high[-5:],
            swing_lows=swings_low[-5:],
            higher_highs=hh,
            higher_lows=hl,
            lower_highs=lh,
            lower_lows=ll,
            trend=trend,
            range_state=not (hh or hl or lh or ll),
            bos_detected=False,
            choch_detected=False,
            structural_failure=False,
            structural_transition=False,
            continuation_likely=trend != "undefined",
    ...
    def _build_support_resistance(
        ...
        candles: List[CanonicalCandle],
        index: int
    ) -> List[SupportResistanceArea]:
        window = candles[max(0, index - 30):index + 1]
        areas: List[SupportResistanceArea] = []

        for i in range(3, len(window) - 3):
            current = window[i]

            is_local_high = (
                current.high > max(c.high for c in window[i - 3:i])
                and current.high > max(
                    c.high for c in window[i + 1:i + 4]
                )
            )
            ...
            absolute_index = max(0, index - 30) + i
            age = index - absolute_index

            freshness = max(
                0.0,
                min(1.0, 1.0 - age / 30.0)
            )

            price_low = current.high * 0.998
            price_high = current.high * 1.002

            latest_close = window[-1].close

            is_broken = latest_close > current.high

            distance_to_level = (
                abs(latest_close - current.high) / current.high
                if current.high != 0
                else float("inf")
            )

            is_retested = distance_to_level < 0.005

            areas.append(
                SupportResistanceArea(
                    price_low=price_low,
    ...
    def _build_momentum(
        ...
        accel = (
            returns[-1] > returns[-2] > 0
            if len(returns) >= 2
            else False
        )

        exhausted = (
            abs(returns[-1]) < abs(returns[-2]) * 0.3
            if len(returns) >= 2
            else False
        )
        ...
        return MomentumState(
            ...
            momentum_decreasing=decreasing,
            accelerating=accel,
            exhausted=exhausted,
    ...
    def _build_regime(
        ...
        if index >= 41:
            prev_return = (
                candles[index - 40].close
                / candles[index - 41].close
                - 1
            )
            prev_vol = abs(prev_return)
        else:
            prev_vol = 0
        ...
        if trending and high_vol:
            ...
            label = "trending_quiet"
        elif high_vol:
            label = "ranging_volatile"
        elif low_vol:
            label = "ranging_quiet"
        elif vol_expanding:
            label = "transition_expanding"
        elif vol_contracting:
            label = "transition_contracting"
    ...
    def _build_context(
        ...
        if recent:
            ...
            pos = (
                (current.close - r_low) / (r_high - r_low)
                if r_high != r_low
                else 0.5
            )
            ...
            if pos > 0.8:
                ...
                location = "upper_middle"
            elif pos < 0.4:
                location = "lower_middle"

        prev_move = "neutral"

        if index > 5:
            base_close = candles[index - 5].close

            if base_close != 0:
                change = (
                    (current.close - base_close)
                    / base_close
                )
                ...
                if change > 0.01:
                    ...
                    prev_move = "strong_down"

        return ContextState(
            instrument=(
                current.instrument
                if hasattr(current, "instrument")
                else "unknown"
            ),
            timeframe=(
                current.timeframe
                if hasattr(current, "timeframe")
                else "unknown"
            ),
            session="unknown",
