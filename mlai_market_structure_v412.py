"""
================================================================================
MLAI v4.1.2 ROBUST CAUSAL PREDICTIVE MARKET STRUCTURE INTELLIGENCE
================================================================================
...
Purpose
-------
This file is a complete replacement for mlai_market_structure_v411.py.

It keeps the causal/walk-forward discipline of v4.1 and adds/strengthens:
    ...
    untouched chronological OOS validation
        +
    strict training/OOS boundary audits
        +
    exact OOS event-diagnostic alignment
        +
    N/A-aware predictive metrics
...
VALIDATION_BIN = "MLAI_V412_CAUSAL_PREDICTIVE_VALIDATION.bin"
VALIDATION_REPORT = "MLAI_V412_CAUSAL_PREDICTIVE_VALIDATION_REPORT.md"
...
class CausalStructureEngine:
    ...
    def _is_confirmed_high(
        ...
        if (
            j < SWING_LEFT
            or j + SWING_RIGHT >= len(self.candles)
        ):
            return False
...
def audit_structure_causality(
    ...
    for s in swings:
        if (
            s.confirmation_index
            < s.pivot_index + SWING_RIGHT
        ):
            reasons.append(
    ...
    for state in states:
        ...
        if state.last_high_index is not None:
            ...
            if (
                high_swing
                and high_swing.confirmation_index > i
            ):
                reasons.append(
        ...
        if state.last_low_index is not None:
            ...
            if (
                low_swing
                and low_swing.confirmation_index > i
            ):
                reasons.append(
...
def audit_training_boundaries(
    ...
    """
    Verify that collect_training_rows includes exactly the valid rows whose
    forward labels are completely contained inside TRAIN.
    """
...
def audit_oos_boundaries(
    ...
    """
    Verify that collect_oos_rows includes exactly the valid OOS rows.

    OOS labels may look beyond the OOS interval, but the target must remain
    inside the loaded dataset.
    """
    for window in windows:
        for horizon in HORIZONS:
            actual_signals, actual_labels = (
...
class KNNModel:
    """
    Frozen training-only nearest-neighbour model.
    """
...
class PredictiveModel:
    ...
    def predict(
        ...
        x = self.scaler.transform_one(
            signal.feature
        )

        p_logistic = (
            self.logistic.predict_proba_one(x)
        )

        p_knn = (
            self.knn.predict_proba_one(x)
        )

        p_state, state_support = (
            self.state.predict(signal)
        )
...
def evaluate_predictions(
    ...
    if used:
        ...
        # No prediction was evaluated.
        # Predictive metrics are unavailable, not zero.
        acc = None
        bal = None
        brier = None
        ll = None
        ...
        # Baseline remains calculable from actual OOS labels.
...
def aggregate_results(
    ...
    for horizon in HORIZONS:
        ...
        output[horizon] = {
            "mean_accuracy": optional_mean(
                accuracies
            ),
            "median_accuracy": optional_median(
                accuracies
            ),
            "std_accuracy": optional_std(
                accuracies
            ),
            "min_accuracy": optional_min(
                accuracies
            ),
            "max_accuracy": optional_max(
                accuracies
            ),
            "mean_balanced_accuracy": optional_mean(
                balanced
            ),
            "mean_edge": optional_mean(
                edges
            ),
            ...
            "mean_brier": optional_mean(
                [
                    row["brier_score"]
                    for row in rows
                ]
            ),
            "mean_log_loss": optional_mean(
                [
                    row["log_loss"]
                    for row in rows
                ]
            ),
            "mean_calibration_error": optional_mean(
                [
...
def event_diagnostics(
    ...
    for window_result in window_results:
        ...
        for horizon in HORIZONS:
            ...
            # This is the exact authoritative OOS row set used by run_window.
            oos_signals, oos_labels = (
                collect_oos_rows(
                    candles,
                    signals,
            ...
            if list(oos_labels) != list(actual):
                raise RuntimeError(
...
def build_report(
    ...
    add(
        "MLAI v4.1.2 ROBUST CAUSAL PREDICTIVE "
        "VALIDATION COMPLETE"
    )
...
def main():
    ...
    validation_artifact = {
        ...
        "audits": {
            ...
            "training_boundary": training_boundary_pass,
            "oos_boundary": oos_boundary_pass,
        },
        "config": {
            "horizons": HORIZONS,
    ...
    report = build_report(
        ...
        oos_boundary_pass=oos_boundary_pass,
    )
